...
LCH_Dict *LCH_DictCreate() {
    ...
    if (self == NULL) {
        LCH_LOG_ERROR("Failed to allocate memory for dict: %s",
                      strerror(errno));
        return NULL;
    }
    ...
    self->buffer =
        (DictElement **)calloc(self->capacity, sizeof(DictElement *));
...
static bool DictCapacity(LCH_Dict *const self) {
    ...
    for (size_t i = 0; i < self->capacity; i++) {
        ...
        LCH_LOG_DEBUG(
            "Moved dict element with key '%s' from index %d to index %d",
            item->key, i, index);
    }
    ...
    LCH_LOG_DEBUG("Expanded dict buffer. New buffer capacity %d/%d",
...
bool LCH_DictSet(LCH_Dict *const self, const char *const key, void *const value,
    ...
    if (self->buffer[index] != NULL) {
        ...
        LCH_LOG_DEBUG(
            "Updated value of dict element with key '%s' at index %zu", key,
            index);
        return true;
    }

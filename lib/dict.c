...
static bool EnsureCapacity(LCH_Dict *const dict) {
  ...
  const bool expand = ((dict->capacity / 100.f) *
                       (dict->in_use - dict->length)) < (1.f - LOAD_FACTOR);

  const size_t new_capacity = (expand) ? dict->capacity * 2 : dict->capacity;
  DictElement **const new_buffer =
      (expand) ? (DictElement **)calloc(new_capacity, sizeof(DictElement *))
               : dict->buffer;
  if (new_buffer == NULL) {
    LCH_LOG_ERROR("calloc(3): Failed to allocate memory: %s", strerror(errno));
    return false;
...
bool LCH_DictSet(LCH_Dict *const dict, const LCH_Buffer *const key, void *const value,
                 void (*destroy)(void *)) {
  assert(dict != NULL);
  assert(dict->buffer != NULL);
  assert(key != NULL);
...
const void *LCH_DictGet(const LCH_Dict *const dict, const LCH_Buffer *const key) {
  assert(dict != NULL);
  assert(dict->buffer != NULL);
  assert(key != NULL);
...
LCH_Dict *LCH_DictSetChangedIntersection(
    ...
    LCH_DuplicateFn duplicate, void (*destroy)(void *),
  ...
  for (size_t i = 0; i < self->capacity; i++) {
    ...
    if (!LCH_DictSet(result, key, left, destroy)) {
      return NULL;
    }
  }
